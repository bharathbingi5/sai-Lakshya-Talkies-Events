...
const AdminDashboard: React.FC = () => {
  ...
  const handleConfirm = async (id: string) => {
    try {
      ...
      setBookings(bookings.map(b => (b.bookingId === id ? { ...b, status: 'confirmed' } : b)));
    } catch {
    ...
    <div className="min-h-screen pt-24 pb-20 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
          <div>
            <h1 className="font-royal text-4xl font-black gold-gradient mb-2 uppercase tracking-widest">
              ROYAL CONSOLE
            </h1>
            <p className="text-gray-400 font-light tracking-wide italic">
              Managing the legacy of Sai Lakshya Talkies.
            </p>
          </div>
          <div className="flex bg-black/40 p-1 rounded-xl border border-white/10 backdrop-blur-md">
            {[
              ...
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as 'bookings' | 'analytics' | 'rooms')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <div className="glass-card p-6 rounded-2xl border-l-4 border-[#d4af37]">
            ...
            <div className="text-3xl font-royal font-black text-white">
              ...
              <div className="glass-card p-8 rounded-2xl border border-[#d4af37]/20">
                <div className="flex justify-between items-center mb-8">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="px-4 py-1 border border-[#d4af37]/40 rounded-full text-xs font-royal hover:bg-[#d4af37]/10"
                    >
                      Today
                    </button>
                    <button
                ...
                <div className="grid grid-cols-7 gap-px bg-[#d4af37]/10 rounded-xl overflow-hidden border border-[#d4af37]/20">
                  {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                    <div
                      key={d}
                      className="bg-black/60 p-4 text-center font-royal text-xs text-gray-500 uppercase tracking-widest"
                    >
                      {d}
                    </div>
                  ))}
                      ...
                      {dayObj && (
                        <>
                          <div className="flex justify-between items-start">
                            <span
                              className={`text-sm ${dayObj.bookings.length > 0 ? 'text-[#d4af37] font-bold' : 'text-gray-500'}`}
                            >
                              {dayObj.day}
                            </span>
                          ...
                          <div className="mt-2 space-y-1">
                            {dayObj.bookings.slice(0, 2).map((b, idx) => (
                              <div
                                key={idx}
                                className="text-[10px] bg-[#d4af37]/10 border border-[#d4af37]/30 text-[#d4af37] px-1 py-0.5 rounded truncate"
                              >
                ...
                {selectedDate && (
                  <div className="mt-6 flex items-center gap-4 text-sm animate-fade-in-down">
                    ...
                    <span className="font-royal font-bold text-[#d4af37]">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-black/60 border-b border-white/10">
                    <tr>
                      <th className="px-6 py-4 font-royal text-xs tracking-widest text-[#d4af37] uppercase">
                        ID
                      </th>
                      <th className="px-6 py-4 font-royal text-xs tracking-widest text-[#d4af37] uppercase">
                        Customer
                    ...
                    {filteredBookings.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="px-6 py-12 text-center text-gray-500 font-light">
                          No bookings found {selectedDate ? 'for this date' : 'in the archives'}.
                        </td>
                      </tr>
                    ) : (
                        ...
                        <tr key={b.id} className="hover:bg-white/5 transition-colors group">
                          ...
                          <td className="px-6 py-4">
                            <div className="text-sm">
                              {rooms.find(r => r.roomId === b.roomId)?.name}
                            </div>
                          ...
                          <td className="px-6 py-4">
                            <div className="flex flex-col">
                              <span className="text-sm font-bold">
                                ₹{b.amountPaid?.toLocaleString() || b.totalPrice.toLocaleString()}{' '}
                                Paid
                              </span>
                              <span className="text-[10px] text-gray-400 uppercase tracking-widest">
                                {b.paymentOption === 'full'
                                  ? 'Full Payment'
                                  : `Advance (Total: ₹${b.totalPrice.toLocaleString()})`}
                              </span>
                            </div>
                          </td>
                          <td className="px-6 py-4">
        ...
        {activeTab === 'analytics' && (
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="glass-card p-8 rounded-2xl border border-[#d4af37]/20">
              <h3 className="font-royal text-xl font-bold mb-8 gold-gradient uppercase tracking-widest">
                ROOM POPULARITY
              </h3>
                  ...
                  <BarChart data={statsData}>
                    ...
                    <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                      {statsData.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={['#bf953f', '#fcf6ba', '#b38728', '#aa771c'][index % 4]}
                        />
                      ))}
                    </Bar>
                  </BarChart>
            ...
            <div className="glass-card p-8 rounded-2xl border border-[#d4af37]/20 flex flex-col justify-center text-center">
              ...
              <h3 className="font-royal text-2xl font-bold mb-4 uppercase tracking-widest">
                INSIGHTS ENGINE
              </h3>
              <p className="text-gray-400 font-light mb-8 max-w-sm mx-auto leading-relaxed">
                Deep intelligence is identifying your growth patterns. Revenue has increased by 12%
                since implementing the 50% advance option.
              </p>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-black/40 rounded-xl border border-white/5">
                  <span className="block text-gray-500 text-[10px] uppercase mb-1 tracking-widest">
                    Peak Slot
                  </span>
                  <span className="block font-royal font-bold text-[#d4af37]">
                    05:00 - 08:00 PM
                  </span>
                </div>
                <div className="p-4 bg-black/40 rounded-xl border border-white/5">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {rooms.map(room => (
              <div key={room.roomId} className="glass-card rounded-2xl overflow-hidden group">
                ...
                <div className="p-6">
                  <h4 className="font-royal font-bold mb-4 uppercase tracking-widest">
                    {room.name}
                  </h4>
            ...
            <div className="glass-card rounded-2xl border-dashed border-2 border-white/10 flex flex-col items-center justify-center p-8 text-gray-500 hover:border-[#d4af37] hover:text-[#d4af37] transition-all cursor-pointer">
              ...
              <span className="font-royal text-sm font-bold uppercase tracking-widest">
                Add New Room
              </span>
            </div>
          </div>
